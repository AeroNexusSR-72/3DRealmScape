...
// Filtra los modelos según la categoría seleccionada
if ($categoriaSeleccionada) {
    $modelosFiltrados = array_filter($modelos, function ($modelo) use ($categoriaSeleccionada) {
        return isset($modelo['categoria']) && $modelo['categoria'] === $categoriaSeleccionada;
    });
} else {
    $modelosFiltrados = $modelos; // Sin filtro, muestra todos los modelos
}
?>
...
<body>
    <div id="contenido">
        ...
        <div id="repertorio">
            ...
            <?php
            $chunkedCarruseles = array_chunk($modelosFiltrados, 8); // Agrupa los productos filtrados en bloques de 8
...
<script>
// Este script actualiza la URL con el filtro seleccionado y recarga la página.
document.querySelectorAll('.dropdown-item').forEach(item => {
    item.addEventListener('click', (e) => {
        e.preventDefault();
        let categoria = e.target.getAttribute('href').split('=')[1];
